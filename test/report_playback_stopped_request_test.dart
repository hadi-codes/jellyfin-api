//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:openapi/api.dart';
import 'package:test/test.dart';

// tests for ReportPlaybackStoppedRequest
void main() {
  // final instance = ReportPlaybackStoppedRequest();

  group('test ReportPlaybackStoppedRequest', () {
    // PlaybackProgressInfoItem item
    test('to test the property `item`', () async {
      // TODO
    });

    // Gets or sets the item identifier.
    // String itemId
    test('to test the property `itemId`', () async {
      // TODO
    });

    // Gets or sets the session id.
    // String sessionId
    test('to test the property `sessionId`', () async {
      // TODO
    });

    // Gets or sets the media version identifier.
    // String mediaSourceId
    test('to test the property `mediaSourceId`', () async {
      // TODO
    });

    // Gets or sets the position ticks.
    // int positionTicks
    test('to test the property `positionTicks`', () async {
      // TODO
    });

    // Gets or sets the live stream identifier.
    // String liveStreamId
    test('to test the property `liveStreamId`', () async {
      // TODO
    });

    // Gets or sets the play session identifier.
    // String playSessionId
    test('to test the property `playSessionId`', () async {
      // TODO
    });

    // Gets or sets a value indicating whether this MediaBrowser.Model.Session.PlaybackStopInfo is failed.
    // bool failed
    test('to test the property `failed`', () async {
      // TODO
    });

    // String nextMediaType
    test('to test the property `nextMediaType`', () async {
      // TODO
    });

    // String playlistItemId
    test('to test the property `playlistItemId`', () async {
      // TODO
    });

    // List<QueueItem> nowPlayingQueue (default value: const [])
    test('to test the property `nowPlayingQueue`', () async {
      // TODO
    });


  });

}
