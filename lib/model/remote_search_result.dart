...
class RemoteSearchResult {
  ...
  @override
  String toString() => 'RemoteSearchResult[name=$name, providerIds=$providerIds, productionYear=$productionYear, indexNumber=$indexNumber, indexNumberEnd=$indexNumberEnd, parentIndexNumber=$parentIndexNumber, premiereDate=$premiereDate, imageUrl=$imageUrl, searchProviderName=$searchProviderName, overview=$overview, albumArtist=$albumArtist, artists=$artists]';
  ...
  /// Returns a new [RemoteSearchResult] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static RemoteSearchResult? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "RemoteSearchResult[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "RemoteSearchResult[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<RemoteSearchResult>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <RemoteSearchResult>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = RemoteSearchResult.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, RemoteSearchResult> mapFromJson(dynamic json) {
    final map = <String, RemoteSearchResult>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RemoteSearchResult.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of RemoteSearchResult-objects as value to a dart map
  static Map<String, List<RemoteSearchResult>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<RemoteSearchResult>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = RemoteSearchResult.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
