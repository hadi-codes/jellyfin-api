...
class AuthenticateUserByName {
  ...
  @override
  String toString() => 'AuthenticateUserByName[username=$username, pw=$pw, password=$password]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AuthenticateUserByName[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AuthenticateUserByName[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AuthenticateUserByName>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<AuthenticateUserByName>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AuthenticateUserByName>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AuthenticateUserByName.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
