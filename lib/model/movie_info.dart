...
class MovieInfo {
  /// Returns a new [MovieInfo] instance.
  MovieInfo({
    this.name,
    this.originalTitle,
    this.path,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is MovieInfo &&
     other.name == name &&
  ...
  @override
  String toString() => 'MovieInfo[name=$name, originalTitle=$originalTitle, path=$path, metadataLanguage=$metadataLanguage, metadataCountryCode=$metadataCountryCode, providerIds=$providerIds, year=$year, indexNumber=$indexNumber, parentIndexNumber=$parentIndexNumber, premiereDate=$premiereDate, isAutomated=$isAutomated]';
  ...
  /// Returns a new [MovieInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MovieInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MovieInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MovieInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MovieInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <MovieInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MovieInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MovieInfo> mapFromJson(dynamic json) {
    final map = <String, MovieInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MovieInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of MovieInfo-objects as value to a dart map
  static Map<String, List<MovieInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<MovieInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MovieInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
