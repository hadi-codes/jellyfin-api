...
class ArtistInfo {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ArtistInfo &&
     other.name == name &&
  ...
  /// Returns a new [ArtistInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ArtistInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ArtistInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ArtistInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ArtistInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ArtistInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ArtistInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ArtistInfo> mapFromJson(dynamic json) {
    final map = <String, ArtistInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ArtistInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ArtistInfo-objects as value to a dart map
  static Map<String, List<ArtistInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ArtistInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ArtistInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
