...
class SeriesTimerInfoDto {
  ...
  /// Returns a new [SeriesTimerInfoDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SeriesTimerInfoDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SeriesTimerInfoDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SeriesTimerInfoDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SeriesTimerInfoDto>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SeriesTimerInfoDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SeriesTimerInfoDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SeriesTimerInfoDto> mapFromJson(dynamic json) {
    final map = <String, SeriesTimerInfoDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SeriesTimerInfoDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SeriesTimerInfoDto-objects as value to a dart map
  static Map<String, List<SeriesTimerInfoDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SeriesTimerInfoDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SeriesTimerInfoDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
