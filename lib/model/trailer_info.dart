...
class TrailerInfo {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TrailerInfo &&
     other.name == name &&
  ...
  @override
  String toString() => 'TrailerInfo[name=$name, originalTitle=$originalTitle, path=$path, metadataLanguage=$metadataLanguage, metadataCountryCode=$metadataCountryCode, providerIds=$providerIds, year=$year, indexNumber=$indexNumber, parentIndexNumber=$parentIndexNumber, premiereDate=$premiereDate, isAutomated=$isAutomated]';
  ...
  /// Returns a new [TrailerInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TrailerInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TrailerInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TrailerInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TrailerInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TrailerInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TrailerInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TrailerInfo> mapFromJson(dynamic json) {
    final map = <String, TrailerInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TrailerInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TrailerInfo-objects as value to a dart map
  static Map<String, List<TrailerInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TrailerInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TrailerInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
