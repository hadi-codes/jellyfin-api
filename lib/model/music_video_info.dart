...
class MusicVideoInfo {
  /// Returns a new [MusicVideoInfo] instance.
  MusicVideoInfo({
    this.name,
    this.originalTitle,
    this.path,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is MusicVideoInfo &&
     other.name == name &&
  ...
  /// Returns a new [MusicVideoInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MusicVideoInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MusicVideoInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MusicVideoInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MusicVideoInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <MusicVideoInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MusicVideoInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MusicVideoInfo> mapFromJson(dynamic json) {
    final map = <String, MusicVideoInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MusicVideoInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of MusicVideoInfo-objects as value to a dart map
  static Map<String, List<MusicVideoInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<MusicVideoInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MusicVideoInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
