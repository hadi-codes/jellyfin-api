...
class BookInfo {
  ...
  /// Returns a new [BookInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BookInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BookInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BookInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BookInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BookInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BookInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BookInfo> mapFromJson(dynamic json) {
    final map = <String, BookInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BookInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BookInfo-objects as value to a dart map
  static Map<String, List<BookInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BookInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BookInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
