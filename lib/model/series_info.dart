...
class SeriesInfo {
  /// Returns a new [SeriesInfo] instance.
  SeriesInfo({
    this.name,
    this.originalTitle,
    this.path,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SeriesInfo &&
     other.name == name &&
  ...
  @override
  String toString() => 'SeriesInfo[name=$name, originalTitle=$originalTitle, path=$path, metadataLanguage=$metadataLanguage, metadataCountryCode=$metadataCountryCode, providerIds=$providerIds, year=$year, indexNumber=$indexNumber, parentIndexNumber=$parentIndexNumber, premiereDate=$premiereDate, isAutomated=$isAutomated]';
  ...
  /// Returns a new [SeriesInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SeriesInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SeriesInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SeriesInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SeriesInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SeriesInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SeriesInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SeriesInfo> mapFromJson(dynamic json) {
    final map = <String, SeriesInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SeriesInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SeriesInfo-objects as value to a dart map
  static Map<String, List<SeriesInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SeriesInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SeriesInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
