...
class BaseItemDto {
  ...
  /// Returns a new [BaseItemDto] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static BaseItemDto? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "BaseItemDto[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "BaseItemDto[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<BaseItemDto>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <BaseItemDto>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = BaseItemDto.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, BaseItemDto> mapFromJson(dynamic json) {
    final map = <String, BaseItemDto>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BaseItemDto.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of BaseItemDto-objects as value to a dart map
  static Map<String, List<BaseItemDto>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<BaseItemDto>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = BaseItemDto.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
