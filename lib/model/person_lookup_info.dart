...
class PersonLookupInfo {
  /// Returns a new [PersonLookupInfo] instance.
  PersonLookupInfo({
    this.name,
    this.originalTitle,
    this.path,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PersonLookupInfo &&
     other.name == name &&
  ...
  @override
  String toString() => 'PersonLookupInfo[name=$name, originalTitle=$originalTitle, path=$path, metadataLanguage=$metadataLanguage, metadataCountryCode=$metadataCountryCode, providerIds=$providerIds, year=$year, indexNumber=$indexNumber, parentIndexNumber=$parentIndexNumber, premiereDate=$premiereDate, isAutomated=$isAutomated]';
  ...
  /// Returns a new [PersonLookupInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PersonLookupInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PersonLookupInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PersonLookupInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PersonLookupInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PersonLookupInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PersonLookupInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PersonLookupInfo> mapFromJson(dynamic json) {
    final map = <String, PersonLookupInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PersonLookupInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PersonLookupInfo-objects as value to a dart map
  static Map<String, List<PersonLookupInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PersonLookupInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PersonLookupInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
