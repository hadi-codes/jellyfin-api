...
class SessionInfoPlayState {
  /// Returns a new [SessionInfoPlayState] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SessionInfoPlayState &&
  ...
  @override
  String toString() => 'SessionInfoPlayState[positionTicks=$positionTicks, canSeek=$canSeek, isPaused=$isPaused, isMuted=$isMuted, volumeLevel=$volumeLevel, audioStreamIndex=$audioStreamIndex, subtitleStreamIndex=$subtitleStreamIndex, mediaSourceId=$mediaSourceId, playMethod=$playMethod, repeatMode=$repeatMode, liveStreamId=$liveStreamId]';
  ...
  /// Returns a new [SessionInfoPlayState] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SessionInfoPlayState? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SessionInfoPlayState[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SessionInfoPlayState[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return SessionInfoPlayState(
  ...
  static List<SessionInfoPlayState>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SessionInfoPlayState>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SessionInfoPlayState.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SessionInfoPlayState> mapFromJson(dynamic json) {
    final map = <String, SessionInfoPlayState>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SessionInfoPlayState.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SessionInfoPlayState-objects as value to a dart map
  static Map<String, List<SessionInfoPlayState>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SessionInfoPlayState>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SessionInfoPlayState.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
