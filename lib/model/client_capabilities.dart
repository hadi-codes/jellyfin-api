...
class ClientCapabilities {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ClientCapabilities &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ClientCapabilities[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ClientCapabilities[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ClientCapabilities>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ClientCapabilities>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ClientCapabilities>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ClientCapabilities.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
