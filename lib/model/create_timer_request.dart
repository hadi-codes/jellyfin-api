...
class CreateTimerRequest {
  /// Returns a new [CreateTimerRequest] instance.
  CreateTimerRequest({
    this.id,
    this.type,
    this.serverId,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is CreateTimerRequest &&
     other.id == id &&
     other.type == type &&
  ...
  /// Returns a new [CreateTimerRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CreateTimerRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CreateTimerRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CreateTimerRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<CreateTimerRequest>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <CreateTimerRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CreateTimerRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CreateTimerRequest> mapFromJson(dynamic json) {
    final map = <String, CreateTimerRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateTimerRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of CreateTimerRequest-objects as value to a dart map
  static Map<String, List<CreateTimerRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<CreateTimerRequest>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CreateTimerRequest.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
