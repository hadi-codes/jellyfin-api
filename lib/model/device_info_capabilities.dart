...
class DeviceInfoCapabilities {
  /// Returns a new [DeviceInfoCapabilities] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is DeviceInfoCapabilities &&
  ...
  /// Returns a new [DeviceInfoCapabilities] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static DeviceInfoCapabilities? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "DeviceInfoCapabilities[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "DeviceInfoCapabilities[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<DeviceInfoCapabilities>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <DeviceInfoCapabilities>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = DeviceInfoCapabilities.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, DeviceInfoCapabilities> mapFromJson(dynamic json) {
    final map = <String, DeviceInfoCapabilities>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DeviceInfoCapabilities.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of DeviceInfoCapabilities-objects as value to a dart map
  static Map<String, List<DeviceInfoCapabilities>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<DeviceInfoCapabilities>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = DeviceInfoCapabilities.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
