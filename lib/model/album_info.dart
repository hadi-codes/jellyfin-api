...
class AlbumInfo {
  ...
  /// Returns a new [AlbumInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static AlbumInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AlbumInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AlbumInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AlbumInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <AlbumInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AlbumInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AlbumInfo> mapFromJson(dynamic json) {
    final map = <String, AlbumInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AlbumInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AlbumInfo-objects as value to a dart map
  static Map<String, List<AlbumInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<AlbumInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AlbumInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
