...
class MusicVideoInfoRemoteSearchQuerySearchInfo {
  /// Returns a new [MusicVideoInfoRemoteSearchQuerySearchInfo] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is MusicVideoInfoRemoteSearchQuerySearchInfo &&
     other.name == name &&
  ...
  @override
  String toString() => 'MusicVideoInfoRemoteSearchQuerySearchInfo[name=$name, originalTitle=$originalTitle, path=$path, metadataLanguage=$metadataLanguage, metadataCountryCode=$metadataCountryCode, providerIds=$providerIds, year=$year, indexNumber=$indexNumber, parentIndexNumber=$parentIndexNumber, premiereDate=$premiereDate, isAutomated=$isAutomated, artists=$artists]';
  ...
  /// Returns a new [MusicVideoInfoRemoteSearchQuerySearchInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MusicVideoInfoRemoteSearchQuerySearchInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MusicVideoInfoRemoteSearchQuerySearchInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MusicVideoInfoRemoteSearchQuerySearchInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MusicVideoInfoRemoteSearchQuerySearchInfo(
  ...
  static List<MusicVideoInfoRemoteSearchQuerySearchInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <MusicVideoInfoRemoteSearchQuerySearchInfo>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MusicVideoInfoRemoteSearchQuerySearchInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MusicVideoInfoRemoteSearchQuerySearchInfo> mapFromJson(dynamic json) {
    final map = <String, MusicVideoInfoRemoteSearchQuerySearchInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MusicVideoInfoRemoteSearchQuerySearchInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of MusicVideoInfoRemoteSearchQuerySearchInfo-objects as value to a dart map
  static Map<String, List<MusicVideoInfoRemoteSearchQuerySearchInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<MusicVideoInfoRemoteSearchQuerySearchInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MusicVideoInfoRemoteSearchQuerySearchInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
