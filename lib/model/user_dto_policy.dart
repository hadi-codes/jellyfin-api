...
class UserDtoPolicy {
  /// Returns a new [UserDtoPolicy] instance.
  UserDtoPolicy({
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UserDtoPolicy &&
  ...
  /// Returns a new [UserDtoPolicy] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UserDtoPolicy? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UserDtoPolicy[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UserDtoPolicy[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return UserDtoPolicy(
  ...
  static List<UserDtoPolicy>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UserDtoPolicy>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UserDtoPolicy.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UserDtoPolicy> mapFromJson(dynamic json) {
    final map = <String, UserDtoPolicy>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UserDtoPolicy.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UserDtoPolicy-objects as value to a dart map
  static Map<String, List<UserDtoPolicy>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UserDtoPolicy>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UserDtoPolicy.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
